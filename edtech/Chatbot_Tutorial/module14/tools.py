...
def calculator(expression: str) -> float:
    ...
    # Handle "square of X" pattern
    if "square of" in lower_expr:
        try:
            # Extract the number after "square of"
            num_str = lower_expr.split("square of")[1].strip()
            # Handle words like "ninety-nine"
            if not any(c.isdigit() for c in num_str):
                # This is a word number, let's use the regular calculator
                pass
            else:
                # Extract just the digits and convert to float
                num = float(''.join(c for c in num_str if c.isdigit() or c == '.'))
                return num * num
        except:
            # If extraction fails, continue with regular evaluation
            pass
...
def get_current_time(timezone: str = "local") -> str:
    ...
    Args:
        timezone: The timezone to get the time for (default: local)
    ...
    if timezone.lower() == "local":
        ...
        # Get UTC time using timezone-aware approach (not deprecated)
        current_time = datetime.datetime.now(datetime.timezone.utc).strftime("%Y-%m-%d %H:%M:%S")
        return f"Current UTC time: {current_time}"
    else:
        # For other timezones, we'd use pytz in a real implementation
        # For now, just return local time with a note
        current_time = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        return f"Current time (showing local time, requested {timezone}): {current_time}"
