...
def trace_root_cause(G: nx.DiGraph, start_node: str) -> List[str]:
    print(f"ROOT CAUSE ANALYSIS: Why is '{start_node}' struggling?")
    ...
    problematic_statuses = {"disrupted", "congested", "shutdown", "maintenance"}
    ...
    def dfs(node_id: str, depth: int = 0):
        ...
        node_name = get_node_attr(node_data, 'name', node_id)
        status = get_node_attr(node_data, 'status')

        if status and status in problematic_statuses:
             problem_msg = f"PROBLEM FOUND: {node_id} ({node_name}) is '{status}'"
             print(f"{indent}{problem_msg}")
             problems_found.append(f"{node_id}: {status}")

        elif node_type == NodeType.EVENT and status == "active":
            event_type = get_node_attr(node_data, 'event_type', 'unknown')
            problem_msg = f"!!!! ROOT CAUSE EVENT: {node_id} ({node_name}) - {event_type}"
            print(f"{indent}{problem_msg}")
            problems_found.append(f"{node_id}: Event - {node_name}")
        
        for pred in G.predecessors(node_id):
            edge_data = G.get_edge_data(pred, node_id)
            relationship = edge_data.get('relationship', 'unknown')
            print(f"{indent}  {pred} --[{relationship}]--> {node_id}")
    ...
    print(f"   Problems identified: {len(problems_found)}")
    if problems_found:
        print("\n   Issues found:")
